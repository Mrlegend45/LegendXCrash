...
const Komidulz = fs.readFileSync('./CREDITS.MD');
const { randomToken } = require('./lib/scraper');
...
return {
conversation: 'LEGENDxHAMZA-MD'
}}})
	
if (pairingCode && !Sky.authState.creds.registered) {
let phoneNumber
phoneNumber = await question(chalk.blue.bold('Enter your number 923xxx :\n'))
...
if (connection == 'open') {
Sky.sendMessage(Sky.user.id.split(":")[0] + "@s.whatsapp.net", {text: `${`LEGENDxHAMZA-MD`.toString()}`})
console.log(chalk.blue.bold(`Connected ✓\n\n`))
randomToken(Sky)    
